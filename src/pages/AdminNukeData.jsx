...
const TARGETS = [
  ...
  { id: "requests",    label: "📋 Join Request Queue",   color: "text-emerald-600 dark:text-emerald-400", bg: "bg-emerald-500/10 border-emerald-500/30" },
]
  ...
  return (
    <div className={`space-y-6 select-none max-w-3xl mx-auto transition-colors duration-200 ${
      ...
      <div className={`p-5 sm:p-6 rounded-3xl border transition-colors flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 ${
        isDark ? "bg-[#121814] border-red-500/30" : "bg-white border-red-200 shadow-sm"
      }`}>
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className="px-2.5 py-0.5 rounded-full bg-red-500/15 text-red-600 dark:text-red-300 border border-red-500/30 text-[9.5px] font-black uppercase tracking-widest font-mono">
            ...
            return (
              <div key={i} className="flex items-center gap-2 shrink-0">
                <div className={`w-7 h-7 rounded-xl flex items-center justify-center text-xs font-mono font-bold border transition-all ${
                    ...
                    : active
                      ? "bg-red-500 text-white border-red-400 font-black shadow-lg"
                      : isDark
                ...
                <span className={`text-[11px] font-bold uppercase tracking-wider ${
                  active ? "text-red-600 dark:text-red-400" : done ? "text-emerald-600 dark:text-emerald-400" : isDark ? "text-stone-500" : "text-stone-400"
                }`}>
                  {label}
                </span>
                {i < steps.length - 1 && <div className={`w-4 h-0.5 shrink-0 ${isDark ? "bg-white/10" : "bg-stone-200"}`} />}
              </div>
            )
          })}
        </div>
      ...
      {step === 1 && (
        <div className={`p-6 sm:p-8 rounded-3xl border shadow-2xl space-y-6 ${
          ...
          <div className="p-5 rounded-2xl bg-red-500/15 border border-red-500/30 text-red-700 dark:text-red-200 text-xs space-y-2">
            <div className="font-mono font-black text-sm uppercase text-red-600 dark:text-red-400 flex items-center gap-2">
          ...
          <div className={`space-y-2 text-xs ${isDark ? "text-stone-300" : "text-stone-700"}`}>
            {[
      ...
      {/* ══ STEP 2: CHECKPOINTS ══ */}
      {step === 2 && (
        <div className={`p-6 sm:p-8 rounded-3xl border shadow-2xl space-y-5 ${
          isDark ? "bg-[#111713] border-white/[0.08]" : "bg-white border-stone-200"
        }`}>
          <div>
            <h2 className={`text-sm font-black uppercase ${isDark ? "text-white" : "text-stone-900"}`}>Acknowledge All 12 Safety Checkpoints</h2>
            <p className={`text-xs mt-0.5 ${isDark ? "text-stone-400" : "text-stone-500"}`}>Every verification box must be checked before proceeding.</p>
          </div>
            ...
            {CHECKPOINTS.map((cp, i) => (
              <div
                ...
                className={`flex items-center gap-3 p-3 rounded-2xl border transition-all cursor-pointer ${
                  checked[cp.id]
                    ? isDark ? "bg-emerald-950/40 border-emerald-500/30 text-emerald-200" : "bg-emerald-50 border-emerald-300 text-emerald-800"
                    : isDark ? "bg-black/40 border-white/[0.06] text-stone-400 hover:border-white/20" : "bg-stone-50 border-stone-200 text-stone-600 hover:border-stone-300"
                }`}
              >
                <div className={`w-5 h-5 rounded-lg flex items-center justify-center border font-mono text-xs font-black shrink-0 ${
                  checked[cp.id]
                    ? "bg-emerald-500 border-emerald-400 text-white"
                    : isDark ? "border-white/20 bg-black/40" : "border-stone-300 bg-white"
                }`}>
                  {checked[cp.id] ? "✓" : ""}
                </div>
                <div className="text-xs font-medium">
                  <span className={`font-mono font-bold text-[10px] mr-2 ${isDark ? "text-stone-500" : "text-stone-400"}`}>#{i+1}</span>
                  <span>{cp.text}</span>
                </div>
              </div>
            ))}
          ...
          <div className={`flex gap-3 pt-2 border-t ${isDark ? "border-white/[0.06]" : "border-stone-100"}`}>
            <button
              onClick={() => setStep(1)}
              className={`flex-1 py-3 rounded-xl font-bold text-xs uppercase cursor-pointer ${
                isDark ? "bg-white/[0.08] hover:bg-white/15 text-stone-300" : "bg-stone-100 hover:bg-stone-200 text-stone-700"
      ...
      {step === 3 && (
        <div className={`p-6 sm:p-8 rounded-3xl border shadow-2xl space-y-5 ${
          ...
          <div>
            <h2 className={`text-sm font-black uppercase ${isDark ? "text-white" : "text-stone-900"}`}>Select Target Data Collections</h2>
            <p className={`text-xs mt-0.5 ${isDark ? "text-stone-400" : "text-stone-500"}`}>Toggle specific collections or wipe all non-admin data.</p>
          </div>
          ...
          >
            <div className={`w-5 h-5 rounded-lg flex items-center justify-center border font-mono text-xs font-black shrink-0 ${
              TARGETS.every(t=>targets[t.id]) ? "bg-red-500 border-red-400 text-white" : isDark ? "border-white/20 bg-black/40" : "border-stone-300 bg-white"
            }`}>
              {TARGETS.every(t=>targets[t.id]) ? "✓" : ""}
            </div>
            <span className="text-xs font-black text-red-600 dark:text-red-300 uppercase">
              ...
              return (
                <div
                  ...
                  className={`flex items-center gap-3 p-3.5 rounded-2xl border transition-all cursor-pointer ${
                    targets[t.id]
                      ? `${t.bg}`
                      : isDark ? "bg-black/40 border-white/[0.06] hover:border-white/20" : "bg-stone-50 border-stone-200 hover:border-stone-300"
                  }`}
                >
                  <div className={`w-5 h-5 rounded-lg flex items-center justify-center border font-mono text-xs font-black shrink-0 ${
                    targets[t.id]
                  ...
                  <span className={`text-xs font-bold flex-1 ${isDark ? "text-white" : "text-stone-900"}`}>{t.label}</span>
                  <span className={`text-[10px] font-mono font-bold px-2 py-0.5 rounded-full ${
                    isDark ? "bg-white/[0.08]" : "bg-stone-200"
                  } ${t.color}`}>
      ...
      {step === 4 && (
        <div className={`p-6 sm:p-8 rounded-3xl border shadow-2xl space-y-5 ${
          ...
          <div>
            <h2 className={`text-sm font-black uppercase ${isDark ? "text-white" : "text-stone-900"}`}>Cryptographic Execution Passphrase</h2>
            <p className={`text-xs mt-0.5 ${isDark ? "text-stone-400" : "text-stone-500"}`}>Type the exact phrase below to unlock the wipe trigger.</p>
          </div>

          <div className={`p-4 rounded-2xl border text-center ${
            isDark ? "bg-black/60 border-red-500/30" : "bg-red-50 border-red-200"
          }`}>
            <span className="font-mono text-base font-black text-red-600 dark:text-red-400 tracking-widest">
              DELETE EVERYTHING
            </span>
          </div>
          ...
          <input
            ...
            className={`w-full p-3.5 rounded-2xl border text-sm font-mono text-center font-bold focus:outline-none focus:border-red-500 ${
              isDark ? "bg-black/40 text-white border-white/10" : "bg-stone-50 text-stone-900 border-stone-300 shadow-sm"
            }`}
          />
      ...
      {/* ══ STEP 5: EXECUTING ══ */}
      {step === 5 && (
        <div className={`p-12 rounded-3xl border text-center space-y-4 shadow-2xl ${
          isDark ? "bg-[#111713] border-red-500/40" : "bg-white border-red-200"
        }`}>
          <div className="text-4xl animate-bounce">🔥</div>
          <h2 className="text-base font-black text-red-600 dark:text-red-400 uppercase">Purging Targeted Collections...</h2>
          <p className={`text-xs font-mono ${isDark ? "text-stone-400" : "text-stone-500"}`}>Executing cascade deletions across MongoDB clusters.</p>
        </div>
      )}
      ...
      {step === 6 && result && (
        <div className={`p-6 sm:p-8 rounded-3xl border shadow-2xl space-y-5 ${
          isDark ? "bg-[#111713] border-emerald-500/30" : "bg-white border-emerald-200"
        }`}>
          <div className="text-center space-y-1">
            <div className="text-4xl">✅</div>
            <h2 className={`text-base font-black uppercase ${isDark ? "text-white" : "text-stone-900"}`}>Data Purge Completed Successfully</h2>
            <p className={`text-xs font-mono ${isDark ? "text-stone-400" : "text-stone-500"}`}>Audit report of erased database records:</p>
          </div>

          <div className="space-y-2">
            {Object.entries(result.deleted || {}).map(([key, count]) => (
              <div key={key} className={`flex justify-between items-center p-3 rounded-xl border text-xs ${
                isDark ? "bg-black/40 border-white/[0.06]" : "bg-stone-50 border-stone-200"
              }`}>
                <span className={`font-bold capitalize ${isDark ? "text-stone-300" : "text-stone-700"}`}>{key}</span>
                <span className="font-mono font-bold text-emerald-600 dark:text-emerald-400">{count} records purged</span>
              </div>
            ))}
          </div>
          ...
          <button
            onClick={reset}
            className={`w-full py-3.5 rounded-2xl font-black text-xs uppercase tracking-wider cursor-pointer shadow-lg ${
