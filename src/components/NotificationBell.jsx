import { useRef, useEffect, useState, useCallback } from "react"
import { createPortal } from "react-dom"
import { useNotifications } from "../context/NotificationContext"
import { useTheme } from "../context/ThemeContext"

/* ── Time ago helper ── */
function timeAgo(isoStr) {
  const diff  = Date.now() - new Date(isoStr).getTime()
  const mins  = Math.floor(diff / 60000)
  const hours = Math.floor(diff / 3600000)
  const days  = Math.floor(diff / 86400000)
  if (mins  < 1)  return "abhi"
  if (mins  < 60) return `${mins}m pehle`
  if (hours < 24) return `${hours}h pehle`
  return `${days}d pehle`
}

/* ── Type → icon/color map ── */
const TYPE_META = {
  new_order:     { icon: "🛒", color: "#3b82f6" },
  dist_approved: { icon: "✅", color: "#10b981" },
  confirmed:     { icon: "🎉", color: "#8b5cf6" },
  rejected:      { icon: "❌", color: "#ef4444" },
  general:       { icon: "🔔", color: "#f59e0b" },
}

export default function NotificationBell({ isMobile = false }) {
  const {
    notifications,
    unreadCount,
    isOpen,
    setIsOpen,
    clearAll,
    deleteNotif,
    markAllRead,
    handleNotifClick,
  } = useNotifications()

  const { isDark } = useTheme()
  const bellBtnRef = useRef(null)
  const [coords, setCoords] = useState(null)
  const [confirmNotif, setConfirmNotif] = useState(null)

  const updateCoords = useCallback(() => {
    if (bellBtnRef.current) {
      const rect = bellBtnRef.current.getBoundingClientRect()
      setCoords({
        top: rect.top,
        bottom: rect.bottom,
        left: rect.left,
        right: rect.right,
        width: rect.width,
        height: rect.height,
      })
    }
  }, [])

  const handleToggle = () => {
    if (!isOpen) {
      updateCoords()
      setIsOpen(true)
    } else {
      setIsOpen(false)
    }
  }

  // Update coords on resize/scroll when open
  useEffect(() => {
    if (!isOpen) return
    const handleReposition = () => updateCoords()
    window.addEventListener("resize", handleReposition, { passive: true })
    window.addEventListener("scroll", handleReposition, { passive: true })
    return () => {
      window.removeEventListener("resize", handleReposition)
      window.removeEventListener("scroll", handleReposition)
    }
  }, [isOpen, updateCoords])

  /* Bell box style */
  const bellBox = {
    position: "relative",
    display: "flex",
    alignItems: "center",
    justifyContent: "center",
    width:  isMobile ? 36 : 38,
    height: isMobile ? 36 : 38,
    borderRadius: 10,
    border: isOpen
      ? "2px solid #f59e0b"
      : unreadCount > 0
        ? "2px solid #fbbf24"
        : isDark ? "2px solid rgba(255,255,255,0.12)" : "2px solid #e2e8f0",
    background: isOpen
      ? isDark ? "rgba(245,158,11,0.2)" : "#fef3c7"
      : unreadCount > 0
        ? isDark ? "rgba(251,191,36,0.15)" : "#fffbeb"
        : isDark ? "rgba(255,255,255,0.06)" : "#f8fafc",
    cursor: "pointer",
    boxShadow: unreadCount > 0 ? "0 0 0 3px rgba(251,191,36,0.2)" : "none",
    transition: "all 0.2s",
  }

  // Dropdown panel style based on portal & positioning
  const getPanelStyle = () => {
    const isSmallScreen = isMobile || (coords && coords.top < 80)
    if (isSmallScreen) {
      return {
        position: "fixed",
        top: coords ? coords.bottom + 8 : 56,
        right: coords ? Math.max(10, window.innerWidth - coords.right - 10) : 10,
        width: "min(360px, calc(100vw - 20px))",
        maxHeight: "calc(100vh - 80px)",
        background: isDark ? "#14171a" : "#ffffff",
        borderRadius: 16,
        boxShadow: isDark
          ? "0 18px 50px rgba(0,0,0,0.65), 0 0 0 1px rgba(255,255,255,0.12)"
          : "0 18px 45px rgba(0,0,0,0.18), 0 0 0 1px #fde68a",
        zIndex: 9999,
        display: "flex",
        flexDirection: "column",
        overflow: "hidden",
      }
    }

    // Desktop sidebar placement: to the right of the sidebar, anchored bottom
    const leftPos = coords ? Math.min(coords.right + 12, window.innerWidth - 374) : 240
    const bottomPos = coords ? Math.max(12, window.innerHeight - coords.bottom - 4) : 20

    return {
      position: "fixed",
      left: Math.max(12, leftPos),
      bottom: bottomPos,
      width: 360,
      maxHeight: "min(500px, calc(100vh - 36px))",
      background: isDark ? "#14171a" : "#ffffff",
      borderRadius: 16,
      boxShadow: isDark
        ? "0 20px 50px rgba(0,0,0,0.7), 0 0 0 1px rgba(255,255,255,0.12)"
        : "0 20px 50px rgba(0,0,0,0.16), 0 0 0 1px #fde68a",
      zIndex: 9999,
      display: "flex",
      flexDirection: "column",
      overflow: "hidden",
    }
  }

  return (
    <>
      {/* ── Bell Button ── */}
      <button
        ref={bellBtnRef}
        onClick={handleToggle}
        title="Notifications"
        style={bellBox}
        type="button"
      >
        <span style={{ fontSize: isMobile ? 18 : 17 }}>🔔</span>
        {unreadCount > 0 && (
          <span style={{
            position: "absolute", top: -6, right: -6,
            background: "#ef4444", color: "#fff",
            fontSize: 9, fontWeight: 800,
            borderRadius: "50%", minWidth: 17, height: 17,
            display: "flex", alignItems: "center", justifyContent: "center",
            padding: "0 3px", border: isDark ? "2px solid #14171a" : "2px solid #fff",
            boxShadow: "0 1px 4px rgba(0,0,0,0.25)",
          }}>
            {unreadCount > 99 ? "99+" : unreadCount}
          </span>
        )}
      </button>

      {/* ── Dropdown Portal ── */}
      {isOpen && createPortal(
        <>
          {/* Backdrop: click outside closes dropdown */}
          <div
            onClick={() => setIsOpen(false)}
            style={{
              position: "fixed",
              inset: 0,
              zIndex: 9998,
              background: "transparent",
            }}
          />

          {/* Panel */}
          <div
            onClick={(e) => e.stopPropagation()}
            style={getPanelStyle()}
          >
            {/* Header */}
            <div style={{
              display: "flex", justifyContent: "space-between", alignItems: "center",
              padding: "12px 16px",
              borderBottom: isDark ? "1px solid rgba(255,255,255,0.08)" : "1px solid #fef3c7",
              background: isDark
                ? "rgba(255,255,255,0.04)"
                : "linear-gradient(135deg, #fffbeb, #fef9ee)",
            }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ fontSize: 16 }}>🔔</span>
                <span style={{
                  fontWeight: 800,
                  fontSize: 14,
                  color: isDark ? "#fbbf24" : "#92400e",
                }}>
                  Notifications
                </span>
                {unreadCount > 0 && (
                  <span style={{
                    background: "#ef4444", color: "#fff",
                    fontSize: 10, fontWeight: 700, borderRadius: 99, padding: "1px 8px"
                  }}>
                    {unreadCount} new
                  </span>
                )}
              </div>
              <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                {notifications.length > 0 && (
                  <button
                    onClick={(e) => { e.stopPropagation(); clearAll() }}
                    style={{
                      fontSize: 11,
                      color: isDark ? "#94a3b8" : "#64748b",
                      background: "none",
                      border: "none",
                      cursor: "pointer",
                      fontWeight: 600,
                      padding: "3px 6px",
                      borderRadius: 6,
                    }}
                  >
                    Clear all
                  </button>
                )}
                <button
                  onClick={(e) => { e.stopPropagation(); setIsOpen(false) }}
                  style={{
                    fontSize: 15,
                    color: isDark ? "#94a3b8" : "#9ca3af",
                    background: "none",
                    border: "none",
                    cursor: "pointer",
                    padding: "3px 5px",
                    borderRadius: 6,
                  }}
                >
                  ✕
                </button>
              </div>
            </div>

            {/* List */}
            <div style={{ overflowY: "auto", flex: 1 }}>
              {notifications.length === 0 ? (
                <div style={{ padding: 36, textAlign: "center" }}>
                  <div style={{ fontSize: 36, marginBottom: 10 }}>🔔</div>
                  <p style={{
                    fontSize: 13,
                    fontWeight: 700,
                    margin: 0,
                    color: isDark ? "#cbd5e1" : "#64748b",
                  }}>
                    Koi notification nahi
                  </p>
                  <p style={{
                    fontSize: 11,
                    margin: "4px 0 0",
                    color: isDark ? "#64748b" : "#94a3b8",
                  }}>
                    Nayi activity hone par yahan dikhega
                  </p>
                </div>
              ) : (
                notifications.map((notif, idx) => {
                  const meta = TYPE_META[notif.type] || TYPE_META.general
                  const isUnread = !notif.read
                  const itemBg = isUnread
                    ? (isDark ? "rgba(245, 158, 11, 0.12)" : "#fffbeb")
                    : (isDark ? "transparent" : "#ffffff")

                  return (
                    <div
                      key={notif._id || idx}
                      onClick={() => setConfirmNotif(notif)}
                      style={{
                        display: "flex", gap: 10,
                        padding: "11px 14px",
                        borderBottom: idx < notifications.length - 1
                          ? (isDark ? "1px solid rgba(255,255,255,0.06)" : "1px solid #f8fafc")
                          : "none",
                        background: itemBg,
                        cursor: "pointer",
                        transition: "background 0.15s",
                      }}
                      onMouseEnter={e => {
                        e.currentTarget.style.background = isDark
                          ? "rgba(255,255,255,0.08)"
                          : "#fef3c7"
                      }}
                      onMouseLeave={e => {
                        e.currentTarget.style.background = itemBg
                      }}
                    >
                      {/* Icon */}
                      <div style={{
                        width: 36, height: 36, borderRadius: "50%",
                        background: meta.color + "18",
                        border: `1.5px solid ${meta.color}40`,
                        display: "flex", alignItems: "center", justifyContent: "center",
                        fontSize: 16, flexShrink: 0,
                      }}>
                        {meta.icon}
                      </div>

                      {/* Text */}
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <p style={{
                          fontSize: 12,
                          fontWeight: notif.read ? 500 : 700,
                          color: isDark ? "#f1f5f9" : "#1e293b",
                          margin: 0,
                          lineHeight: 1.45,
                        }}>
                          {notif.message}
                        </p>
                        {/* Sender name */}
                        {notif.senderName && (
                          <p style={{
                            fontSize: 10,
                            color: isDark ? "#a78bfa" : "#7c3aed",
                            margin: "2px 0 0",
                            fontWeight: 600,
                          }}>
                            👤 {notif.senderName} ({notif.senderRole})
                          </p>
                        )}
                        <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 4 }}>
                          <p style={{
                            fontSize: 10,
                            color: isDark ? "#94a3b8" : "#94a3b8",
                            margin: 0,
                          }}>
                            {timeAgo(notif.createdAt)}
                          </p>
                          {notif.targetPage && (
                            <span
                              onClick={(e) => {
                                e.stopPropagation()
                                setConfirmNotif(notif)
                              }}
                              style={{
                                fontSize: 9,
                                color: isDark ? "#60a5fa" : "#2563eb",
                                background: isDark ? "rgba(59,130,246,0.15)" : "#eff6ff",
                                borderRadius: 4,
                                padding: "1px 6px",
                                fontWeight: 600,
                                cursor: "pointer",
                              }}
                            >
                              👆 Tap to view →
                            </span>
                          )}
                        </div>
                      </div>

                      {/* Right side: delete btn + unread dot */}
                      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6, flexShrink: 0 }}>
                        <button
                          onClick={(e) => {
                            e.stopPropagation()
                            deleteNotif(notif._id)
                          }}
                          style={{
                            background: "none",
                            border: "none",
                            cursor: "pointer",
                            fontSize: 13,
                            color: isDark ? "#64748b" : "#94a3b8",
                            lineHeight: 1,
                            padding: "2px 4px",
                            borderRadius: 4,
                          }}
                          title="Hatao"
                        >
                          ✕
                        </button>
                        {!notif.read && (
                          <div style={{
                            width: 8, height: 8, borderRadius: "50%",
                            background: "#f59e0b", flexShrink: 0,
                          }} />
                        )}
                      </div>
                    </div>
                  )
                })
              )}
            </div>
          </div>
        </>,
        document.body
      )}

      {/* ── Redirect Confirmation Popup Portal ── */}
      {confirmNotif && createPortal(
        <div
          data-confirm-popup="true"
          style={{
            position: "fixed", inset: 0, zIndex: 10000,
            background: "rgba(0,0,0,0.5)",
            backdropFilter: "blur(4px)",
            display: "flex", alignItems: "center", justifyContent: "center",
          }}
          onClick={() => setConfirmNotif(null)}
        >
          <div
            style={{
              background: isDark ? "#181c20" : "#ffffff",
              color: isDark ? "#f1f5f9" : "#1e293b",
              borderRadius: 16,
              padding: "24px 20px",
              width: "min(340px, 88vw)",
              boxShadow: isDark
                ? "0 20px 50px rgba(0,0,0,0.8), 0 0 0 1px rgba(255,255,255,0.12)"
                : "0 10px 40px rgba(0,0,0,0.2)",
              textAlign: "center",
            }}
            onClick={e => e.stopPropagation()}
          >
            <div style={{ fontSize: 36, marginBottom: 8 }}>📬</div>
            <div style={{
              fontSize: 15,
              fontWeight: 800,
              color: isDark ? "#f8fafc" : "#1e293b",
              marginBottom: 6,
            }}>
              {confirmNotif.message || "Notification"}
            </div>
            {confirmNotif.senderName && (
              <div style={{ fontSize: 12, color: isDark ? "#94a3b8" : "#64748b", marginBottom: 4 }}>
                From: {confirmNotif.senderName}
              </div>
            )}
            {confirmNotif.targetPage && (
              <div style={{
                fontSize: 12,
                color: isDark ? "#cbd5e1" : "#475569",
                marginBottom: 16,
                background: isDark ? "rgba(255,255,255,0.06)" : "#f8fafc",
                borderRadius: 8,
                padding: "6px 10px",
              }}>
                📍 Page: <b>{confirmNotif.targetPage}</b>
              </div>
            )}
            <div style={{ display: "flex", gap: 10 }}>
              <button
                onClick={() => setConfirmNotif(null)}
                style={{
                  flex: 1, padding: "11px 0", borderRadius: 10,
                  border: isDark ? "1.5px solid rgba(255,255,255,0.12)" : "1.5px solid #e2e8f0",
                  background: isDark ? "rgba(255,255,255,0.05)" : "#fff",
                  color: isDark ? "#94a3b8" : "#64748b",
                  fontWeight: 700, fontSize: 14, cursor: "pointer",
                }}
              >
                ❌ Cancel
              </button>
              <button
                onClick={() => {
                  handleNotifClick(confirmNotif)
                  setConfirmNotif(null)
                }}
                style={{
                  flex: 1, padding: "11px 0", borderRadius: 10, border: "none",
                  background: "#2563eb", color: "#fff", fontWeight: 700, fontSize: 14, cursor: "pointer",
                }}
              >
                ✅ Haan, Jao!
              </button>
            </div>
          </div>
        </div>,
        document.body
      )}
    </>
  )
}

